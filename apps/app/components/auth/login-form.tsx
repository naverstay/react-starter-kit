...
function AuthFormContent({ onSuccess, className, isExternallyLoading }: AuthFormContentProps) {
  const { email, isDisabled, error, showOtpInput, setEmail, handleSuccess, handleError, sendOtp, resetOtpFlow } = useLoginForm({
    onSuccess,
    isExternallyLoading,
  });
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)}>
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-muted-foreground text-balance">Sign in or create your account</p>
      </div>
      ...
      <PasskeyLogin onSuccess={handleSuccess} onError={handleError} isDisabled={isDisabled} />
      ...
      {!showOtpInput ? (
        <form onSubmit={sendOtp} className="grid gap-3">
          <Input
            ...
            required
          />
          <Button type="submit" variant="secondary" className="w-full" disabled={isDisabled || !email}>
            <Mail className="mr-2 h-4 w-4" />
            Continue with email
          </Button>
        </form>
      ) : (
        <OtpVerification email={email} onSuccess={handleSuccess} onError={handleError} onCancel={resetOtpFlow} isDisabled={isDisabled} />
      )}
    </div>
  );
}
...
export function LoginForm({ className, variant = "page", showTerms, onSuccess, isLoading, ...props }: LoginFormProps) {
  ...
  if (variant === "modal") {
    return (
      <div className={cn("flex flex-col gap-4", className)} {...props}>
        <AuthFormContent onSuccess={onSuccess} isExternallyLoading={isLoading} />
        {shouldShowTerms && (
